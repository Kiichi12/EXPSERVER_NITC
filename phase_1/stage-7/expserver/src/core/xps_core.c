...
xps_core_t *xps_core_create() {
    ...
    if (core == NULL) {
        logger(LOG_ERROR, "xps_core_create()", "Failed to allocate memory for xps_core_t");
        return NULL;
    }

    xps_loop_t *loop = malloc(sizeof(xps_loop_t));
    if (loop == NULL) {
        logger(LOG_ERROR, "xps_core_create()", "Failed to allocate memory for xps_loop_t");
        free(core);
        return NULL;
    }
    ...
    logger(LOG_INFO, "xps_core_create()", "created core");
...
void xps_core_destroy(xps_core_t *core) {
    ...
    // Destroy connections
    logger(LOG_DEBUG, "xps_core_destroy()", "destroying connections");
    ...
    // Destroy listeners
    logger(LOG_DEBUG, "xps_core_destroy()", "destroying listeners");
    for (int i = 0; i < core->listeners.length; i++) {
        xps_connection_t *listener = core->listeners.data[i];
        if (listener != NULL)
            xps_connection_destroy(listener); // modification of xps_connection_destroy() will be look at later
        }
    ...
    // Free core instance
    logger(LOG_DEBUG, "xps_core_destroy()", "freeing core");
    free(core);

    logger(LOG_INFO, "xps_core_destroy()", "destroyed core");
}
