...
int main() {
    ...
    epoll_ctl(epoll_fd, EPOLL_CTL_ADD, listen_sock_fd, &event); //add listening socket to epoll
    ...
    // Loop to make it an Iterative server
    while(1) {
      ...
      for(int i = 0; i < n_ready_fds; i++) {
        ...
        if(curr_fd == listen_sock_fd) {
          ...
          epoll_ctl(epoll_fd, EPOLL_CTL_ADD, conn_sock_fd, &client_event);
        }
        else {
          // Handle client data
